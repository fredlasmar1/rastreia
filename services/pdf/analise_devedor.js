...
function secaoSituacaoPatrimonial(doc, y, dados) {
  ...
  if (!imoveis.length && !veiculos.length && !empresas.length) {
    return boxEmIntegracao(doc, y,
      ...
      'Para levantamento patrimonial completo (imóveis + todos os veículos + contas), utilizar o produto Investigação Patrimonial (R$ 149).'
    );
  }
...
function secaoCapacidadePagamento(doc, y, dados, pedido) {
  ...
  // Os scores (QUOD + Boa Vista/SCPC) aparecem na caixa "SCORE DE CRÉDITO —
  // 2 BUREAUS" logo abaixo (secaoScoreCredito); não repetimos aqui.
  return y + 6;
}
